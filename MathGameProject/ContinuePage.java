...
public class ContinuePage extends MainFile {

    private int nextLevelCount;

    ContinuePage(int nextLevelCount) {
    ...
    public void start(Stage primaryStage) {
        ...
        Text referringText = new Text("Referring to the next level\n\n");
        ...
        nextLevelButton.setOnAction(e -> {
            try {
                String className = "Level" + (nextLevelCount) + "Page";
                Class<?> curClass = Class.forName(className);
                Application levelPage = (Application) curClass.newInstance();
                levelPage.start(primaryStage);
            }
            catch (Exception ex) { 
                ex.printStackTrace();
                throw new IllegalStateException("Unexpected value: " + nextLevelCount, ex);
            }
        });
